...
\paragraph{À quelle(s) problématique(s) l'article répond ?} Les techniques développées permettent une détection plus prompte d'une dérive. Le modèle adaptable permet de bonnes performances malgrès le nombre faible d'observations.
...
\paragraph{Résumé :} L'article propose une solution d'apprentissage automatique sur un problème contrebalancé en présence d'une dérive conceptuelle. Le traitement des données contrebalancé ce fait par la teechnique SMOTE, qui permets une création de points sythétiques de la classe minoritaire. La détection de la dérive ce fait par une surveillance des performances du modèle par des seuils, on a affaire a une detection par seuil statistiques de 4 métriques dérivé des prédictions du modèle et de la vérité.
